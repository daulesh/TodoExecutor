...
async def run_planner_agent(user_id: uuid.UUID, user_message: str) -> AgentRunResult:
    """
    ...
    try:
        ...
        return AgentRunResult(text=f"Error executing agentic planner loop: {str(e)}")

    if not result.text:
        return AgentRunResult(text="No planning response generated by the agent.", usage=result.usage)
    return result
