...
async def run_rescheduler_agent(user_id: uuid.UUID, user_message: str) -> AgentRunResult:
    """
    ...
    try:
        ...
        return AgentRunResult(text=f"Error executing agentic rescheduler loop: {str(e)}")

    if not result.text:
        return AgentRunResult(text="No rescheduling response generated by the agent.", usage=result.usage)
    return result
